...
`fnpack.json` 的 `apps` 就是一个**字典**，键是应用标识，值是该应用的全部信息与版本列表 ——
**一个源里想放多少个软件就放多少个**，和 [shuangji66/FnDepot](https://github.com/shuangji66/FnDepot)
（同一个文件里放了近百个应用）用的是同一套 schema：

```json
{
  "schema_version": "2",
  "source_info": { "name": "...", "author": "...", "homepage": "..." },
  "apps": {
    "dbx":   { "display_name": "...", "releases": { "0.6.19": { "packages": { "x86": { ... } } } } },
    "另一个软件": { "display_name": "...", "releases": { "1.0.0":  { "packages": { "x86": { ... } } } } }
  }
}
```

往同一个源里再加一个软件，只需三步（本仓库的 CI 与工具都会自动带上）：

1. 在仓库里加一个同名目录放商店素材：`<应用标识>/ICON.PNG`、`<应用标识>/README.md`
   （可选 `Preview/*.png` 截图），`icon_url` / `readme_url` 指向它；
2. 把它的 fpk 提交进 Release（CI 里加一个 job，或用 `gh release upload`），
   再用 `tools/gen-fnpack.py --app <应用标识> ...` 把这条 release 合并进 `fnpack.json`；
3. 推上去，用户在 FnDepot 里刷新就能看到两个应用。

> 当前本源里只有 **DBX** 一个应用，是因为到目前为止只做了这一个包 —— 不是格式限制。
> 想加第二个（比如某个自建的影视库/阅读站/工具），按上面三步走即可，不需要改结构。
